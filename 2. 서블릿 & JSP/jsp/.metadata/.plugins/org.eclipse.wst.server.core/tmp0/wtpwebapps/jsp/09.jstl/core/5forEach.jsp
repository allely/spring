...
<!-- 일반 for문의 경우 current, index, var 3가지 값이 동일하다. -->
	...
	<h2>1~100까지의 정수 중 홀수의 합을 구해보자.</h2>
	...
	%>
	<br>
	<!-- 영역에 저장되지 않았기 때문에 스크립틀릿으로 -->
	<table border="1">
	<c:forEach items="<%=rgba %>" var="color" varStatus="loop">
		<!-- index : 0부터 시작
			 count : 1부터 시작 (반복한 수만큼 증가) -->
		<c:set value="${loop.index mod 2 eq 1 }" var="odd"></c:set>
		odd : ${odd }
		<tr class="${odd}">
			<td>color : ${color } </td>
	...
	%>
	<!-- el표현언어를 통해 사용하기 위해서
		controller를 통해서 화면을 출력할 경우, 내장객체의 영역에 저장해주고 있으므로 따로 선언하지 않아도 된다. -->
	<c:set var="list" value="<%=list %>"></c:set>
	<ul>
		<c:forEach items="${list }" var="bDto">
			<li>제목 : ${bDto.title } / 작성자 : ${bDto.id } / 작성일 : ${bDto.postdate }</li>
		</c:forEach>
	</ul>
