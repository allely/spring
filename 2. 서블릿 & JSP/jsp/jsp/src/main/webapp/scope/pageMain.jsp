...
<body>
	<!-- 내장객체의 영역
			...
			세션이 모여서 하나의 어플리케이션(application)을 이룬다.
		page영역 : 동일한 페이지에서만 공유
		request : 하나의 요청에 의해 호출된 페이지 및 포워드 페이지
		session영역 : 클라이언트가 처음 웹브라우저로 접속한 후 웹브라우저를 닫을 때까지
		application영역 : 웹어플리케이션(웹서버)가 종료될 때까지
	 -->
	 ...
	 <!-- removeAttribute메서드는 값이 없는 경우 오류를 발생하지 않는다.
	 	getAttribute 메서드는 값이 없는 경우 null 을 반환한다. -->
